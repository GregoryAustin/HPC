...
void calculateDistances3D(float *Ax, float *Ay, float *Az, int k, int fullSet[], int listA[], int aC, int full) {
    float d1[full], d2[full], d3[full], d4[full], d5[full], d6[full], sum[full];
    int index[full];

    // biggest and smallest value sorted by x
    mergeSort(Ax, fullSet, 0, full-1);
    int p1 = fullSet[0]; 
    int p2 = fullSet[full-1];

    // biggest and smallest value sorted by y
    mergeSort(Ay, fullSet, 0, full-1);
    int p3 = fullSet[0];
    int p4 = fullSet[full-1];

    // biggest and smallest value sorted by z
    mergeSort(Az, fullSet, 0, full-1);
    int p5 = fullSet[0];
    int p6 = fullSet[full-1];

    //  Find distance of each point in p array from p1 and put its square in the d1 array. 
    for (int i = 0; i < full; ++i) 
        d1[i] = getSquaredDistance(Ax[p1], Ax[fullSet[i]], Ay[p1], Ay[fullSet[i]], Az[p1], Az[fullSet[i]]);

    //  Find distance of each point in p array from p2 and put its square in the d2 array. 
    for (int i = 0; i < full; ++i)
        d2[i] = getSquaredDistance(Ax[p2], Ax[fullSet[i]], Ay[p2], Ay[fullSet[i]], Az[p2], Az[fullSet[i]]);

    //  Find distance of each point in p array from p2 and put its square in the d2 array. 
    for (int i = 0; i < full; ++i) 
        d3[i] = getSquaredDistance(Ax[p3], Ax[fullSet[i]], Ay[p3], Ay[fullSet[i]], Az[p3], Az[fullSet[i]]);

    //  Find distance of each point in p array from p2 and put its square in the d2 array. 
    for (int i = 0; i < full; ++i) 
        d4[i] = getSquaredDistance(Ax[p4], Ax[fullSet[i]], Ay[p4], Ay[fullSet[i]], Az[p4], Az[fullSet[i]]);

    //  Find distance of each point in p array from p2 and put its square in the d2 array. 
    for (int i = 0; i < full; ++i) 
        d5[i] = getSquaredDistance(Ax[p5], Ax[fullSet[i]], Ay[p5], Ay[fullSet[i]], Az[p5], Az[fullSet[i]]);;
    
    //  Find distance of each point in p array from p2 and put its square in the d2 array. 
    for (int i = 0; i < full; ++i) 
        d6[i] = getSquaredDistance(Ax[p6], Ax[fullSet[i]], Ay[p6], Ay[fullSet[i]], Az[p6], Az[fullSet[i]]);
    ...
    for (int i = 0; i < full; ++i) {
        for (int j = i+1; j <= i+100 && j < full; ++j) {
            int pointA = fullSet[index[i]];
            int pointB = fullSet[index[j]];  
            if(checkDifferentSets(pointA, pointB, listA, aC)) {
...
int main(int argc, char **argv) {
    ...
    int * full = array_concat(setA, aC, setB, bC, int_size); 
    // printf("Combined sets");
    // print_array(full, aC + bC);
    ...
    for (int i = 0; i < dcd->nsets; i++) {
        ...
        */
        ...
        calculateDistances3D(dcd->x, dcd->y, dcd->z, k, full, setA, aC , aC+bC);  
